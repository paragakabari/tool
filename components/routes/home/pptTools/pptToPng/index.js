...
export default function PptToPNG({ Data }) {
  ...
  const handleDownload = (url) => {
    ...
    saveAs(downloadData, `Document.png`);
    setLoader(false);
  ...
  const handleChange = (acceptedFiles) => {
    ...
    if (file.size > 10e6) {
      ...
      if (FileExtension == "ppt" || FileExtension == "pptx") {
        ...
        let convertApi = ConvertApi.auth("VjoMJ9FtUxiA0tDQ");
        let params = convertApi.createParams();
        params.add("File", file);
        let result = convertApi.convert("ppt", "png", params);
            ...
            <div className="cusLoader">
              <div className="spinner">
                ...
                <div className="button center-button-cancel">
                  <button onClick={() => handleProcessCancel()}>cancel  <Loader /> </button>
                </div>
              </div>
            </div>
          )}
